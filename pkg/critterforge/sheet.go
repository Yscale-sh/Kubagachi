...
type KeyedSheetSpec struct {
	...
	// ReferencePNGs are additional canonical character references attached to
	// the sheet generation call. ReferencePNG, when set, is treated as the
	// first reference.
	ReferencePNGs [][]byte
}
...
func generateSheet(ctx context.Context, opts SheetOptions, spec KeyedSheetSpec) error {
	...
	// Anchor the sheet to canonical character art when the manifest supplies
	// references, so every status frame is the same mascot instead of being
	// re-imagined from text.
	refs := make([][]byte, 0, len(spec.ReferencePNGs)+1)
	if len(spec.ReferencePNG) > 0 {
		refs = append(refs, spec.ReferencePNG)
	}
	refs = append(refs, spec.ReferencePNGs...)
...
func SheetSpecsFromInput(input *InputManifest) []KeyedSheetSpec {
	...
	for _, ic := range input.Critters {
		...
		spec := KeyedSheetSpec{
			...
			ReferencePNGs: append([][]byte(nil), ic.ReferencePNGs...),
		}
		if ic.Reference != "" {
