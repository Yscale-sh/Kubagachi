...
// SecretValue carries a single secret entry with its base64-encoded form and
// the decoded UTF-8 string. The json tags ("b64", "decoded") match the wire
// format consumed by the web UI. This type is defined here (not in internal/k8s)
// so the tui package stays free of any client-go import.
type SecretValue struct {
	B64     string `json:"b64"`
	Decoded string `json:"decoded"`
}

// PortForward describes an active pod port-forward. JSON tags match the wire
// format served by /api/portforward/*.
...
// CustomResourceRef is a compact, client-go-free reference to a custom
// resource instance.
...
// HelmRevision describes a single Helm release revision for the history endpoint.
...
// HelmDetail carries the values, manifest, and notes for a Helm release revision.
...
type Actions interface {
	...
	DeleteResource(ctx context.Context, apiVersion, kind, namespace, name string) error
	ScaleResource(ctx context.Context, apiVersion, kind, namespace, name string, replicas int32) error
	RestartResource(ctx context.Context, apiVersion, kind, namespace, name string) error
	...
	// ObjectYAML fetches the full YAML for any cluster object. apiVersion may
	// be empty; namespace is empty for cluster-scoped resources.
	ObjectYAML(ctx context.Context, apiVersion, kind, namespace, name string) (string, error)
	CustomResources(ctx context.Context, group, version, resource, namespace string) ([]CustomResourceRef, error)
	// ApplyYAML server-side-applies the given YAML document to the cluster and
	// returns the applied object re-serialized as YAML. In demo mode it returns
	// the input yaml unchanged without writing to any cluster.
	ApplyYAML(ctx context.Context, yaml string) (string, error)
	// SecretData returns per-key base64 + decoded values for a Secret.
	SecretData(ctx context.Context, namespace, name string) (map[string]SecretValue, error)
	// CordonNode marks a node as unschedulable (cordon=true) or schedulable
	// (cordon=false) via a merge-patch on spec.unschedulable.
	CordonNode(ctx context.Context, name string, cordon bool) error
	// PortForwardStart establishes a port-forward from localPort on 127.0.0.1 to
	// remotePort on the named pod. localPort 0 lets the kernel choose a free port.
	// The forward's lifetime is owned by the registry, not the ctx.
	PortForwardStart(ctx context.Context, namespace, pod string, remotePort, localPort int) (PortForward, error)
	// PortForwardStop stops a forward by its registry ID.
	PortForwardStop(id string) error
	// PortForwardList returns a snapshot of all active forwards.
	PortForwardList() []PortForward
	// HelmHistory returns all decoded revisions for a named release, sorted
	// descending by revision number.
	HelmHistory(ctx context.Context, namespace, name string) ([]HelmRevision, error)
	// HelmReleaseDetail returns the user-supplied values (YAML), manifest, and
	// notes for a specific release revision.
	HelmReleaseDetail(ctx context.Context, namespace, name string, revision int) (HelmDetail, error)
	// HelmAvailable reports whether the helm CLI binary is on PATH.
	HelmAvailable() bool
	// HelmRollback rolls back a release to the given revision via the helm CLI.
	// Returns helm's combined output on both success and error.
	HelmRollback(ctx context.Context, namespace, name string, revision int) (string, error)
	// HelmUninstall removes a release via the helm CLI.
	// Returns helm's combined output on both success and error.
	HelmUninstall(ctx context.Context, namespace, name string) (string, error)
}
