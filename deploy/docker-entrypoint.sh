...
# 0. Explicit demo mode — highest precedence. For a public demo / any deploy that
#    should serve the fake habitat regardless of a mounted SA token or kubeconfig.
if [ "${KUBAGACHI_MODE:-}" = "demo" ]; then
  echo "kubagachi: KUBAGACHI_MODE=demo — serving --demo (no cluster access)" >&2
